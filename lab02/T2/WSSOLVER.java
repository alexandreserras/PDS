...
public class WSSOLVER {
	...
	public static void main(String[] args) {
		...
		int maxsize = 40;
		try {
			Scanner ficheiro = new Scanner(new File("filename.txt"));
...
/*		O bloco abaixo verifica se o puzzle respeita as condicionantes necess�rias */	
			...
			for(int i = 2; i < FirstLen+1;i++) {//Uso este ciclo for para verificar se o puzzle � quadrado
...
/* O bloco acima verifica se o puzzle respeita as condicionantes necess�rias */		
			
			
/* O bloco abaixo extrai as palavras para achar na WS e verifica se respeitam as condi��es */				
...
/* O bloco acima extrai as palavras para achar na WS e verifica se respeitam as condi��es */			
			
			//System.out.println(lista_palavras);
			//System.out.println(puzzle);
			maxsize = FirstLen; // a este ponto j� verifiquei que a WS � funcional, logo sei que FirstLen � a dimens�o de puzzle
			ficheiro.close();
		}
 ...
 * 					sopadeletras que conter� o puzzle
 * 					solucao que conter� . (eventualmente ser� preenchido com as palavras encontradas)
 */
...
// Procurar as palavras na sopa de letras
			...
			for (int linha = 0; linha < sopadeletras.length; linha++) {
				for (int coluna = 0; coluna < sopadeletras[linha].length; coluna++) {
					if (primeira == sopadeletras[linha][coluna]) { //quando encontro a letra inicials
						...
						//procurar para tras e para cima caso haja espa�o
						...
						//procurar para cima caso haja espa�o
	...
	private static void searchupleft(int linha, int coluna, List<String> palavrasFound, char[][] solucao, char[][] sopadeletras, String palavra) {
		...
		for(int i=1; i<palavra.length();i++) {
			if(sopadeletras[linha-i][coluna-i] == palavra.charAt(i) && i == palavra.length()-1 && !palavrasFound.contains(palavra)) {//verifica��o caso esteja na ultima letra
				palavrasFound.add(palavra);
				for(int j=0; j<palavra.length();j++) {
					solucao[linha-j][coluna-j] = sopadeletras[linha-j][coluna-j];//Escrever a solu��o
	...
	private static void searchleft(int linha, int coluna, List<String> palavrasFound, char[][] solucao, char[][] sopadeletras, String palavra) {
		...
		int foundcolumn = coluna+1;
		for(int i=1; i<palavra.length();i++) {
			if(sopadeletras[linha][coluna-i] == palavra.charAt(i) && i == palavra.length()-1 && !palavrasFound.contains(palavra)) {//verifica��o caso esteja na ultima letra
				palavrasFound.add(palavra);
				for(int j=0; j<palavra.length();j++) {
					solucao[linha][coluna-j] = sopadeletras[linha][coluna-j];//Escrever a solu��o
					//System.out.println("success");
				}
	...
	private static void searchdownleft(int linha, int coluna, List<String> palavrasFound, char[][] solucao, char[][] sopadeletras, String palavra) {
		...
		for(int i=1; i<palavra.length();i++) {
			if(sopadeletras[linha+i][coluna-i] == palavra.charAt(i) && i == palavra.length()-1 && !palavrasFound.contains(palavra)) {//verifica��o caso esteja na ultima letra
				palavrasFound.add(palavra);
				for(int j=0; j<palavra.length();j++) {
					solucao[linha+i][coluna-j] = sopadeletras[linha+i][coluna-j];//Escrever a solu��o
					//System.out.println("success");
				}
		...
		private static void searchdown(int linha, int coluna, List<String> palavrasFound, char[][] solucao, char[][] sopadeletras, String palavra) {
			...
			int foundcolumn = coluna+1;
			for(int i=1; i<palavra.length();i++) {
				if(sopadeletras[linha+i][coluna] == palavra.charAt(i) && i == palavra.length()-1 && !palavrasFound.contains(palavra)) {//verifica��o caso esteja na ultima letra
					palavrasFound.add(palavra);
					for(int j=0; j<palavra.length();j++) {
						solucao[linha+j][coluna] = sopadeletras[linha+j][coluna];//Escrever a solu��o
						//System.out.println("success");
					}
