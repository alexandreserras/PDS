...
public class JGalo extends JFrame implements ActionListener {
	...
	public static void main(String args[]) {
		char firstPlayer;
		try {
			firstPlayer = args[0].toUpperCase( Locale.ROOT ).charAt( 0 );
		}
		catch (IndexOutOfBoundsException e){
			firstPlayer = 'X';
		}
		if ( firstPlayer == 'O' || firstPlayer == 'X')
			new JGalo(new Jogo( firstPlayer));
		else
			throw new IllegalArgumentException("Valid option X(x) or O(o) defaults to X");
